...
VALUE method_rb_Softmax_eval (VALUE self, VALUE parameter_1) {
	...
	if (CLASS_OF(parameter_1) == rb_RealVector::rb_class()) {
		...
		RealMatrix (*(r->getModel()))(*(parameter_1_converted->getData()))_casted = (*(r->getModel()))(*(parameter_1_converted->getData()));
		return wrap_pointer<rb_RealMatrix>(rb_RealMatrix::rb_class(), new rb_RealMatrix((*(r->getModel()))(*(parameter_1_converted->getData()))_casted));
		...
		RealMatrix (*(r->getModel()))(parameter_1_converted)_casted = (*(r->getModel()))(parameter_1_converted);
		return wrap_pointer<rb_RealMatrix>(rb_RealMatrix::rb_class(), new rb_RealMatrix((*(r->getModel()))(parameter_1_converted)_casted));
	} else if (TYPE(parameter_1) == T_ARRAY) {
