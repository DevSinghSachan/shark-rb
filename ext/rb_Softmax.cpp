...
VALUE method_rb_Softmax_set_set_structure (VALUE self, VALUE parameter_1) {
	rb_Softmax * r;
...
VALUE method_rb_Softmax_eval (VALUE self, VALUE parameter_1) {
	...
	if (CLASS_OF(parameter_1) == rb_RealVector::rb_class()) {
		...
		return wrap_pointer<rb_UnlabeledData>(rb_UnlabeledData::rb_class(), new rb_UnlabeledData(r->getModel()->eval(*(parameter_1_converted->getData()))));
		...
		return wrap_pointer<rb_UnlabeledData>(rb_UnlabeledData::rb_class(), new rb_UnlabeledData(r->getModel()->eval(parameter_1_converted)));
	} else if (TYPE(parameter_1) == T_ARRAY) {
...
void Init_rb_Softmax () {
	rb_define_method(rb_Softmax::rb_class(), "set_structure=", (rb_method) method_rb_Softmax_set_set_structure, 1);
