...
VALUE method_rb_Softmax_eval (VALUE self, VALUE parameter_1) {
	...
	if (CLASS_OF(parameter_1) == rb_RealVector::rb_class()) {
		...
		return wrap_pointer<rb_UnlabeledData>(rb_UnlabeledData::rb_class(), new rb_UnlabeledData((*(r->getModel()))(*(parameter_1_converted->getData()))));
		...
		return wrap_pointer<rb_UnlabeledData>(rb_UnlabeledData::rb_class(), new rb_UnlabeledData((*(r->getModel()))(parameter_1_converted)));
	} else if (TYPE(parameter_1) == T_ARRAY) {
