#include "rb_RealMatrixReferenceReference.h"
...
rb_RealMatrixReferenceReference::rb_RealMatrixReferenceReference(RealMatrix const& _data) {
	data = _data;
}
RealVector rb_RealMatrixReference::get_row(int i) {
	return row(data, i);
}
RealVector rb_RealMatrixReference::get_column(int i) {
	return column(data, i);
}

void rb_RealMatrixReference::fill(double filling) {
	std::fill(data->storage(), data->storage()+(data->size1()*data->size2()), filling);
}

VALUE method_realmatrix_get_sqrt (VALUE self) {
	rb_RealMatrixReference *m;
	Data_Get_Struct(self, rb_RealMatrixReference, m);
	m->data = sqrt(m->data);
	return self;
}
	...
	if (TYPE(rb_other) == T_DATA) {
		if (CLASS_OF(rb_other) == rb_optimizer_realmatrix_klass) {
			...
			if (other->data->size1() == m->data->size1() && other->data.size2() == m->data->size2()) {
				return wrap_pointer<rb_RealMatrixReference>(
					...
					new rb_RealMatrixReference(m->data + other->data)
					);
			} else if (other->data.size2() == m->data.size1() && other->data.size1() == m->data.size2()) {
				return wrap_pointer<rb_RealMatrixReference>(
					rb_optimizer_realmatrix_reference_klass,
					new rb_RealMatrixReference(m->data + trans(other->data))
					);
			} else {
		...
		if (RARRAY_LEN(rb_other) > 0 && TYPE(rb_ary_entry(rb_other, 0)) == T_ARRAY) {
			...
			if (other.size1() == m->data.size1() && other.size2() == m->data.size2()) {
				return wrap_pointer<rb_RealMatrixReference>(
					rb_optimizer_realmatrix_reference_klass,
					new rb_RealMatrixReference(m->data + other)
					);
			} else if (other.size2() == m->data.size1() && other.size1() == m->data.size2()) {
				return wrap_pointer<rb_RealMatrixReference>(
					rb_optimizer_realmatrix_reference_klass,
					new rb_RealMatrixReference(m->data + trans(other))
					);
			} else {
				rb_raise(rb_eArgError, "The Matrices you are summing don't have the same dimensions, so their sum is infeasible using our current (human) technology.");
			}
...
VALUE method_realmatrix_add_equals (VALUE self, VALUE rb_other) {
	...
	Data_Get_Struct(self, rb_RealMatrixReference, m);
	if (TYPE(rb_other) == T_DATA) {
		if (CLASS_OF(rb_other) == rb_optimizer_realmatrix_reference_klass) {
			rb_RealMatrixReference *other;
			Data_Get_Struct(rb_other, rb_RealMatrixReference, other);
			if (other->data.size1() == m->data.size1() && other->data.size2() == m->data.size2()) {
				m->data += other->data;
			} else if (other->data.size2() == m->data.size1() && other->data.size1() == m->data.size2()) {
...
VALUE method_realmatrix_multiply (VALUE self, VALUE multiplier) {
	...
	if (TYPE(multiplier) == T_FLOAT || TYPE(multiplier) == T_FIXNUM) {
		return wrap_pointer<rb_RealMatrixReference>(
			rb_optimizer_realmatrix_reference_klass,
			new rb_RealMatrixReference((m->data) * NUM2DBL(multiplier))
			);
	} else if (TYPE(multiplier) == T_DATA) {
		VALUE multiplier_klass = CLASS_OF(multiplier);
		if (multiplier_klass == rb_optimizer_realmatrix_reference_klass) {
			...
			return wrap_pointer<rb_RealMatrixReference>(
				rb_optimizer_realmatrix_reference_klass,
				new rb_RealMatrixReference(prod(m->data, multi_matrix->data))
				);
		...
		if (RARRAY_LEN(multiplier) > 0 && TYPE(rb_ary_entry(multiplier, 0)) == T_ARRAY) {
			...
			return wrap_pointer<rb_RealMatrixReference>(
				rb_optimizer_realmatrix_reference_klass,
				new rb_RealMatrixReference(prod(m->data, multi_matrix))
			);
...
VALUE method_realmatrix_negate (VALUE self) {
	...
	return wrap_pointer<rb_RealMatrixReference>(
		rb_optimizer_realmatrix_reference_klass,
		new rb_RealMatrixReference(-(m->data))
	);
}
VALUE method_realmatrix_allocate (VALUE klass) {
	return wrap_pointer<rb_RealMatrixReference>(
			rb_optimizer_realmatrix_reference_klass,
			new rb_RealMatrixReference()
			);
}
...
void Init_RealMatrix () {
	...
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "sqrt", (rb_method)method_realmatrix_get_sqrt,0);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "to_a", (rb_method)method_realmatrix_to_ary, 0);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "to_realvectors", (rb_method)method_realmatrix_to_realvectors, 0);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "*", (rb_method)method_realmatrix_multiply, 1);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "*=", (rb_method)method_realmatrix_multiply_equals, 1);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "/", (rb_method)method_realmatrix_divide, 1);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "+", (rb_method)method_realmatrix_add, 1);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "+=", (rb_method)method_realmatrix_add_equals, 1);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "length", (rb_method)method_realmatrix_length, 0);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "size", (rb_method)method_realmatrix_length, 0);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "transpose", (rb_method)method_realmatrix_transpose, 0);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "~", (rb_method)method_realmatrix_transpose, 0);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "size1", (rb_method)method_realmatrix_size1, 0);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "size2", (rb_method)method_realmatrix_size2, 0);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "stride1", (rb_method)method_realmatrix_stride1, 0);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "stride2", (rb_method)method_realmatrix_stride2, 0);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "row", (rb_method)method_realmatrix_get_row, 1);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "column", (rb_method)method_realmatrix_get_column, 1);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "col", (rb_method)method_realmatrix_get_column, 1);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "fill", (rb_method)method_realmatrix_fill, 1);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "clear", (rb_method)method_realmatrix_clear, 0);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "-@", (rb_method)method_realmatrix_negate,0);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "[]", (rb_method)method_realmatrix_query,2);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "resize", (rb_method)method_realmatrix_resize, 2);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "[]=", (rb_method)method_realmatrix_insert,3);
	rb_define_alloc_func(rb_optimizer_realmatrix_reference_klass, (rb_alloc_func_t) method_realmatrix_allocate);
	rb_define_method(rb_optimizer_realmatrix_reference_klass, "initialize", (rb_method)method_realmatrix_initialize,-1);
